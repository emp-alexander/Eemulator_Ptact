...
import x.gui.XCellInfoPanel;
import x.gui.XMainPanel;
import x.gui.XMapPanel;
import x.logic.XMath;
import x.logic.XRandomGenerator;
...
public class XForce {
	...
	private static boolean tryToDie(long cellData, int y, int x) {
		...
		boolean decision = XRandomGenerator.generateBoolean(2 * (XUcfCoder.decodeHumanAge(cellData) / 300) - 60);
		if (decision) {
			clearHuman(y, x);
		XStatistic.peopleDied++;
	...
	private static boolean tryToEat(long cellData, int y, int x) {
		...
		// f(x) = 120 - 2*x;
		boolean decision = XRandomGenerator.generateBoolean(120 - 2 * XUcfCoder.decodeHumanSatiety(cellData));
		if (decision) {
				...
				for (int xShift = -1; xShift < 2; xShift++) {
					...
					if (isCellInMapRange(yTarget, xTarget)) {
						...
						if (fruitsTarget != 0) {
							...
							//XMainPanel.eventsInfoPanel.update(date, "Eat. ");
							return true;
						}
					}
			}	
	...
	private static boolean tryToSleep(long cellData, int y, int x) {
		// f(x): f(0..10) >= 100, f(60..64) <= 0;
		// f(x) = 120 - 2*x;
		boolean decision = XRandomGenerator.generateBoolean(120 - 2 * XUcfCoder.decodeHumanEnergy(cellData));
		if (decision) {
			XMapPanel map = XMainPanel.mapPanel;
			...
			for (int yShift = -1; yShift < 2; yShift++) {
				for (int xShift = -1; xShift < 2; xShift++) {
					...
					if (isCellInMapRange(yTarget, xTarget)
						...
						boolean decision = XRandomGenerator.generateBoolean(30);
						if (decision) {
...
// HUMAN - GIVE BIRTH
			...
			for (int yShift = -1; yShift < 2; yShift++) {
				for (int xShift = -1; xShift < 2; xShift++) {
					...
					if (isCellInMapRange(yTarget, xTarget) && map.getHumanTypeAt(yTarget, xTarget) == XUcfCoder.HUMAN_TYPE_EMPTY) {
						map.setHumanTypeAt(XRandomGenerator.generateBoolean() ? XUcfCoder.HUMAN_TYPE_MAN : XUcfCoder.HUMAN_TYPE_WOMAN, yTarget, xTarget);
	...
	private static boolean tryToMove(long cellData, int y, int x, int yShift, int xShift) {
		...
		if (yShift == 0 && xShift == 0) {
			yShift = XRandomGenerator.generateInteger(-1, 1);	
			xShift = XRandomGenerator.generateInteger(-1, 1);
		}
		if (yShift != 0 || xShift != 0) {
			int yTarget = y + yShift;
			int xTarget = x + xShift;
	...
	private static void tryToMakeFruits(long cellData, int y, int x) {
		if (date % 30 == 0) {
			XMapPanel map = XMainPanel.mapPanel;
			map.setPlantFruitsAt(map.getPlantFruitsAt(y, x) + XRandomGenerator.generateInteger(5, 15), y, x);
		}
	}
	...
	private static void tryToDropFruit(long cellData, int y, int x) {
		...
		// 0.01 * 0.50 = 0.005
		boolean decision = XRandomGenerator.generateBoolean(1) && XRandomGenerator.generateBoolean(50);
		if (decision) {
			map.setPlantFruitsAt(map.getPlantFruitsAt(y, x) - 1, y, x);
			int yTarget = y + XRandomGenerator.generateInteger(-2, 2);
			int xTarget = x + XRandomGenerator.generateInteger(-2, 2);
